...
public class FedoraReaderPureImp implements FedoraReaderPure {
    ...
    private List<String> possiblyGetRequestedObjectsFromFedoraLimitedByRows(String type, int rows, String responseXML) {
        var pidList = fedoraReaderXmlHelper.getPidList(responseXML);
    ...
    private List<String> possiblyGetObjectsFromFedoraLimitByRows(int rows, List<String> pidList) {
        ...
        return pidList.subList(0, upperLimitOfPidList).stream().map(this::readObject).collect(Collectors.toList());
    }
